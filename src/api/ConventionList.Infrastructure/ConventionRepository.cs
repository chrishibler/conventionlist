...
using ConventionList.Infrastructure.Mapping;
    ...
    : IConventionRepository
{
    ...
    public async Task Add(Convention con, Geocoordinate? coord = null)
    {
        con.Position = GeocoordinateTypeConverter.ToPoint(coord);
        await db.Conventions.AddAsync(con);
    }

    public void Update(Convention con, Geocoordinate? coord = null)
    {
        con.Position = GeocoordinateTypeConverter.ToPoint(coord);
